    ...
    while 1:
        try:
            ...
            if station!=res["@StationID"]:
                ...
                if x1[0]!="BL01":
                    return
                os.makedirs("./data/BL/{}/eastbound/json/".format(out[x1[0]]["@updateTime"].split(" ")[0]), exist_ok=True)
                f=open("data/BL/{}/eastbound/json/{}.json".format(out[x1[0]]["@updateTime"].split(" ")[0],out[x1[0]]["@updateTime"].split(" ")[1]),"w+")
            ...
            if retry_cnt>=10:
                if len(x1)>0:
                   os.remove("data/BL/{}/eastbound/json/{}.json".format(out[x1[0]]["@updateTime"].split(" ")[0],out[x1[0]]["@updateTime"].split(" ")[1]))
                return
            retry_cnt+=1
            time.sleep(1)

    #os.system("/usr/bin/python3.7 draw.py data/BL/{}/eastbound/json/{}.json".format(out[x1[0]]["@updateTime"].split(" ")[0],out[x1[0]]["@updateTime"].split(" ")[1]))
    return 
