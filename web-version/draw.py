...
car_1 = [float(data[i]["@Cart1W"]) for i in x1]
car_2 = [float(data[i]["@Cart2W"]) for i in x1]
car_3 = [float(data[i]["@Cart3W"]) for i in x1]
car_4 = [float(data[i]["@Cart4W"]) for i in x1]
car_5 = [float(data[i]["@Cart5W"]) for i in x1]
car_6 = [float(data[i]["@Cart6W"]) for i in x1]
...
import os
os.makedirs("./data/BL/{}/eastbound/image/".format(data[x1[0]]["@updateTime"].split(" ")[0]), exist_ok=True)

plt.savefig('./data/BL/{}/eastbound/image/{}.png'.format(data[x1[0]]["@updateTime"].split(" ")[0],data[x1[0]]["@updateTime"].split(" ")[1]))
